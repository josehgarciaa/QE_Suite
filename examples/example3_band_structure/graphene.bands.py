...
type="automatic"
scf = SCF().set_k_points(type, hexagonal.get_kpoints(type=type)).set_pseudopot_dir("../SSSP")
pw_input = PWInput(calculation = scf, system=graphene  );
output_file = "qe_suite.scf.inp";
...
output_file = "qe_suite.bands.inp";
pw_input.write(output_file)
#import subprocess
#proc= subprocess.run(["pw.x -inp "+output_file], shell=True);
